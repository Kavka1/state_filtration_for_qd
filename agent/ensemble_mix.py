...
class Ensemble(object):
    ...
    def _compute_disagree_reward(self, obs_filt: np.array, next_obs_filt: np.array, action: np.array) -> List[float]:
        ...
        for k in range(self.num_primitive):
        ...
        intrinsic_reward /= self.num_primitive
        return intrinsic_reward.tolist()

    def rollout_update(self) -> Dict:
        for k, primitive in enumerate(self.primitives):
            primitive_policy_optimizer              = self.primitive_policy_optimizers[k]
            primitive_inverse_model_optimizer       = self.primitive_inverse_model_optimizers[k]
    ...
    def evaluate(self, env, num_episodes: int) -> float:
        rewards_across_primitive = []
        for k in range(self.num_primitive):
            primitive = self.primitives[k]
            reward = 0
            for i_episode in range(num_episodes):
                ...
                step = 0
                while not done:
                    action = primitive.decision(obs, False)
                    action = np.clip(action, -env.action_bound, env.action_bound)
                    next_obs, r, done, info = env.step(action)
                    reward += r
                    obs = next_obs
                    step += 1
            rewards_across_primitive.append(reward / num_episodes)
        return rewards_across_primitive
    ...
    def save_policy(self, remark: str) -> None:
        ...
        for k in range(self.num_primitive):
            ...
            torch.save(self.primitives[k].policy.state_dict(), path)
        print(f"- - - - Policy of primitive saved to {model_path} - - - -")
    
    def save_inverse_model(self, remark: str) -> None:
        model_path = self.exp_path + 'model/'
        confirm_path_exist(model_path)
        for k in range(self.num_primitive):
            path = model_path + f'idm_{k}_{remark}'
            torch.save(self.primitives[k].inverse_model.state_dict(), path)
        print(f"- - - - Inverse model of primitive saved to {model_path} - - - -")
