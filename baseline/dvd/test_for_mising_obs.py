...
def main(path: str, remark: str, obs_index: List[int], csv_path: str) -> None:
    ...
    for k in range(num_primitive):
        ...
        all_workers.append(Worker.remote(model_path, config['model_config'], 10))
...
if __name__ == '__main__':
    ...
    for obs_index in [
        ...
        for seed in [10, 20, 30, 40, 50, 60, 70, 80]:
            main(
                path=f'/home/xukang/Project/state_filtration_for_qd/results_for_dvd/{env}-{seed}/',
                remark='best',
                obs_index=obs_index,
                csv_path=f'/home/xukang/Project/state_filtration_for_qd/statistic/dvd/{env}-defective_sensor-{csv_mark}-{seed}.csv'
            )