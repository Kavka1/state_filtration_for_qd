...
class DvD_PPO(object):
    def __init__(self, config: Dict) -> None:
        ...
        self.tradeoff               = config['tradeoff']           # the tradeoff of the intrinsic reward
    ...
    def _compute_action_disagreement_reward(self, obs: np.array, k: int) -> List[float]:
        obs_tensor  = torch.from_numpy(obs).float().to(self.device)
        self_a_mean = self.policies[k].act(obs_tensor, False)
        rewards     = 0
        for i in range(self.z_dim):
            if i == k:
                continue
            a_mean  = self.policies[i].act(obs_tensor, False)
            a_diff  = torch.exp(torch.square(self_a_mean - a_mean)).sum(-1, keepdim=True) / 2
            rewards += a_diff

        rewards /= (self.z_dim - 1)
    ...
    def rollout_update(self) -> Dict:
        for k in range(self.z_dim):
            ...
            for traj_dict in all_trajectory:
                ...
                r_in_seq = self._compute_action_disagreement_reward(
                    np.stack(traj_dict['obs'], 0),  k
                )
            ...
            log_loss_value, log_loss_policy = 0, 0
            update_count = 0
